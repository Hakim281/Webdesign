# Weekly Logbook

## Week 4

Completed work:

- reviewed server-side programming requirements
- prepared PHP login and registration flow
- confirmed session-based access control
- confirmed database connection setup
- added a contact/viewing request form with backend validation
- stored contact submissions in MySQL
- prepared `Week4db.sql` for versioned database backup

Evidence to capture:

- screenshots of login, registration, and validation
- screenshot of database tables in phpMyAdmin
- screenshot of GitHub commit history

## Week 5

Completed work:

- prepared MySQL database scripts
- confirmed table creation files
- confirmed dynamic frontend integration
- prepared `Week5db.sql` for versioned database backup

Evidence to capture:

- screenshots of create, read, update, and delete operations
- screenshot of database table records




